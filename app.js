...
const INSTITUTIONAL_COLORS = [
    { name: 'Azul Marino Principal', hex: '#242C4F' },
    { name: 'Azul Secundario', hex: '#436589' },
    { name: 'Amarillo Institucional', hex: '#E6BB62' },
    { name: 'Gris', hex: '#a0a0a0' },
    { name: 'Gris oscuro', hex: '#808080' },
    { name: 'Gris Oscuro 2', hex: '#5A5A5A' },
    { name: 'Negro de Apoyo', hex: '#141414' }
];

function addCoverFreeText() {
    const uniqueId = Math.random().toString(36).substr(2, 9);
    state.pdfConfig.coverElements.push({
        id: uniqueId, type: 'free', value: '',
        fontSize: 15, isBold: false, color: '#436589', alignment: 'left'
    });
    renderCoverElements();
}
...
function addCoverDynamicField() {
    const uniqueId = Math.random().toString(36).substr(2, 9);
    state.pdfConfig.coverElements.push({
        id: uniqueId, type: 'dynamic', value: '',
        fontSize: 15, isBold: true, color: '#436589', alignment: 'left'
    });
    renderCoverElements();
}
...
function updateCoverElement(id, field, val) {
    ...
    if (el) {
        el[field] = val;
        if (field !== 'value') renderCoverElements();
    }
}
...
function renderCoverElements() {
    ...
    const colorOpts = (selected) => INSTITUTIONAL_COLORS.map(c =>
        `<option value="${c.hex}" ${selected === c.hex ? 'selected' : ''}>${c.name} (${c.hex})</option>`
    ).join('');

    const sizeOpts = (selected) => [10, 12, 14, 15, 17, 20, 24, 30, 36].map(s =>
        `<option value="${s}" ${selected == s ? 'selected' : ''}>${s}pt</option>`
    ).join('');

    const alignOpts = (selected) => ['left', 'center', 'right'].map(a =>
        `<option value="${a}" ${selected === a ? 'selected' : ''}>${a === 'left' ? 'Izquierda' : a === 'center' ? 'Centro' : 'Derecha'}</option>`
    ).join('');
    ...
    container.innerHTML = state.pdfConfig.coverElements.map(el => {
        ...
        const controlsHtml = `
                    <div class="cover-controls">
                        <select class="cover-control-select" onchange="updateCoverElement('${el.id}', 'fontSize', parseInt(this.value))" title="Tamaño de Fuente">
                            ${sizeOpts(el.fontSize || 15)}
                        </select>
                        <select class="cover-control-select" onchange="updateCoverElement('${el.id}', 'color', this.value)" title="Color">
                            ${colorOpts(el.color || '#436589')}
                        </select>
                        <select class="cover-control-select" onchange="updateCoverElement('${el.id}', 'alignment', this.value)" title="Alineación">
                            ${alignOpts(el.alignment || 'left')}
                        </select>
                        <label class="cover-control-checkbox" title="Negrita">
                            <input type="checkbox" ${el.isBold ? 'checked' : ''} onchange="updateCoverElement('${el.id}', 'isBold', this.checked)">
                            <b>B</b>
                        </label>
                    </div>
                `;

        return `<div class="cover-element-card">
                            <div style="display: flex; gap: 8px; align-items: flex-start;">
                                <div style="flex: 1;">
                                    <div style="font-size:10px; color:var(--text3); margin-bottom:2px;">
                                        ${el.type === 'free' ? 'Texto Libre' : 'Campo Dinámico'}
                                    </div>
                                    ${innerHtml}
                                    ${controlsHtml}
                                </div>
                                <button class="row-del" onclick="removeCoverElement('${el.id}')" style="margin-top: 14px;">✕</button>
                            </div>
                        </div>`;
    }).join('');
}
...
function buildDocDefinition(headerImgBase64) {
    ...
    if (state.pdfConfig.coverElements && state.pdfConfig.coverElements.length > 0) {
        ...
        // Función para validar si es un HEX válido
        ...
        state.pdfConfig.coverElements.forEach(el => {
            ...
            if (textVal) {
                coverStack.push({
                    ...
                    bold: el.isBold,
                    color: isValidHex(el.color) ? el.color : '#436589',
